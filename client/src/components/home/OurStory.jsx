...
const OurStory = ({ story }) => {
  ...
  return (
    <section className="relative bg-charcoal text-white overflow-hidden min-h-[300px] flex items-center">
      {story.image && (
        <div className="absolute inset-0 z-0">
          <img src={driveImg(story.image)} alt={story.heading || 'Our Story'} className="w-full h-full object-cover opacity-35" />
      ...
      <div className="relative z-10 max-w-6xl mx-auto px-4 py-16 sm:py-20 grid grid-cols-1 md:grid-cols-2 gap-10 md:gap-16 items-center w-full">
        ...
        <div>
          <p className="text-cream-200/90 text-sm leading-relaxed mb-6">{teaser}</p>
          <Link
            to="/about"
            className="inline-block border border-cream-200/40 text-cream-100 text-xs tracking-widest px-6 py-3 hover:border-gold hover:text-gold transition-colors backdrop-blur-sm"
          >
            READ OUR STORY
          </Link>
        </div>
