...
const Contact = () => {
  ...
  useEffect(() => {
    publicAxios.get('/layout').then(({ data }) => {
      setContactInfo(data.data.websiteContent?.contactInfo);
      setHero(data.data.websiteContent?.contactPage || null);
    });
  }, []);
  ...
  return (
    <div className="bg-cream">
      <StaticPageHero
        heading={hero?.heroHeading || 'Contact Us'}
      ...
      <div className="max-w-7xl mx-auto px-4 py-14">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 sm:gap-8 max-w-4xl mx-auto">
          <div className="bg-white border border-cream-200 p-6 sm:p-7">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <textarea
                ...
                value={form.message}
                onChange={handleChange}
                className="w-full px-4 py-3 border border-cream-200 text-sm focus:outline-none focus:ring-1 focus:ring-brand resize-none"
              />
              {message && (
              ...
              <button
                type="submit"
                disabled={submitting}
                className="w-full bg-brand hover:bg-brand-dark text-white text-xs tracking-widest py-3 transition-colors disabled:opacity-60"
              >
